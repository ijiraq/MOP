...
def plant_kbos(filename, psf, kbos, shifts, prefix):
    ...
    addstar = open(f"{filename}.add", 'w')
    # transform KBO locations to this frame using the shifts provided.
    w = get_wcs(shifts)
    header = fits.open(filename.replace('.fits', '.mopheader'))[0].header
    ...
    # iraf.images.chpix(fk_image, fk_image, 'short')
...
def plant(expnums, ccd, rmin, rmax, ang, width, number=10,
          mmin=2.0, mmax=26.5, version='s', dry_run=False, force=True):
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="THIS OPTION IS IGNORED")
    ...
    for ccd in ccdlist:
        plant(args.expnums,
              ...
              version=version, force=args.force)
